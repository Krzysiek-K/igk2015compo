...
public:
	...
	bool LoadF(const char *path,D3DFORMAT fmt);
...
public:
	...
	bool GetPixels(int &w,int &h,std::vector<DWORD> &data);	// 32 & 64 bpp

	bool RequiresFill() { bool r=needs_fill; needs_fill=false; return r; }
...
public:
	DevCubeRenderTarget();
	DevCubeRenderTarget(int _format,int _size,int _flags=0);
	~DevCubeRenderTarget();

	IDirect3DCubeTexture9 *GetTexture() { return rt; }
	IDirect3DSurface9 *GetSurface(int face);
	IDirect3DSurface9 *GetSurfaceLevel(int face,int level);
	
	void SetParameters(int _format,int _size,int _flags=0);
...
public:
	...
	void		SetFloat (const char *name,float v)							{ if(fx) fx->SetFloat(name,v); }
	void		SetFloat2(const char *name,const base::vec2 &v)				{ if(fx) fx->SetFloatArray(name,&v.x,2); }
	void		SetFloat3(const char *name,const base::vec3 &v)				{ if(fx) fx->SetFloatArray(name,&v.x,3); }
	void		SetFloat4(const char *name,const base::vec4 &v)				{ if(fx) fx->SetFloatArray(name,&v.x,4); }
    void		SetTexture(const char *name,IDirect3DBaseTexture9 *tex)		{ if(fx) fx->SetTexture(name,tex); }
    void		SetMatrix(const char *name,const D3DXMATRIX &mtx)			{ if(fx) fx->SetMatrix(name,&mtx); }
    void		SetMatrixTranspose(const char *name,const D3DXMATRIX &mtx)	{ if(fx) fx->SetMatrixTranspose(name,&mtx); }
    void		CommitChanges()												{ if(fx) fx->CommitChanges(); }
...
public:
	...
	void SetCubeRenderTargetLevel(int id,DevCubeRenderTarget *rt,int face,int level);
	...
	void DrawScreenQuad(const base::vec2 &p1,const base::vec2 &p2,float z=0);
	void DrawScreenQuadTC(const base::vec2 &p1,const base::vec2 &p2,const base::vec2 &tc1,const base::vec2 &tc2,float z=0);
